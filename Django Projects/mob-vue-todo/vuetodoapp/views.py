...
def get_completed_todoitems(request):
    completed_items = ToDoItem.objects.filter(completed=True)

    num_pages = 1
    total = completed_items.count()
    ...
    return JsonResponse({'completed_items': complete_item_data, "num_pages": num_pages, 'total': total})
...
def get_todoitems(request):
    ...
    # print(test)
    return JsonResponse({"todoitems":todo_data, 'num_pages': num_pages, 'total': total })
