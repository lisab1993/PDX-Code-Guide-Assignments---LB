...
//Student Notes:
//tripleAll will be a function with xs as its parameter
//tripleAll will use map to affect every value in the array, and return them in a new array
//map's argument is another function that takes x as a parameter, and multiplies it by 3
...
//Student Notes:
//upperCaseNames is a function with people (an array) as a parameter
//people.map will affect every item within the array
//map will run a function that takes i as a parameter, and makes it upper case
const upperCaseNames = people => people.map(i => i.name.toUpperCase())
...
//Student Notes:
//userLinks takes users(an array of objects) as a parameter
//for each element of the array, generate an a tag, with the href set to include the username
//set the inner text of the a tag to the user's username
...
//Student Notes:
//the function has applicants(an array) as a parameter
//first, we will ensure that the boolean for advancedJSCourse is true as well as the age being 18 or over
//next, we will use .map to iterate through this new array and return the emails of the targeted students
...
// The accumulator will start with the value of the first element in the array.
...
//Student Notes:
//applicants is the paramenter(an array of objects)
//the reduce method takes accumulatorObject(an empty object) and arrayElement(the elements of the array, which are objects in this case) as parameters
//on each iteration, use the spread operator on the accumulatorObject along with the array Element to combine both objects
//the {} at the end means that our accumulator's (accumulatorObject) default value is a blank object
const mergeObjects = applicants => applicants.reduce((accumulatorObject, arrayElement) => ({ ...accumulatorObject, ...arrayElement }), {}); 
