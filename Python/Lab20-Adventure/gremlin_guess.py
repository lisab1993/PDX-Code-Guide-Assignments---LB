...
def guess_the_number():
    ...
    if older_guess == computer_selection:#if the user is correct on the first try, end the game
        print(f"You got it! The answer was {computer_selection}. You got it on the first try. Not only are you free, but you're lucky too!")  
    else:
    ...
    while num_guesses > 0:
            if newer_guess == computer_selection:#I do want lines 14 - 22 to loop
                print(f"You got it! The answer was {computer_selection}. You're free!!!")
                return (f"You got it in {num_guesses} tries")
                break
            else:
